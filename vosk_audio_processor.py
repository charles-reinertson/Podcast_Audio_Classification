...
class VoskAudioProcessor:
    ...
    def process(self, urls_and_durations, sample_length=30, language='en',
                sample_rate=16000):
        """
        ...
        :param sample_length: the length of the two audio samples used in processing
        ...
        for index, (url, duration) in tqdm(urls_and_durations.iterrows(),
                                           total=len(urls_and_durations)):
            try:
                starts = get_random_start_times(duration, sample_length)
                clips = save_audio_clips(url, sample_rate, starts, sample_length)
...
def save_audio_clips(url, sample_rate, starts, clip_length):
    ...
    for start, destination_filename in zip(starts, clip_filenames):
        subprocess.run(['ffmpeg', '-loglevel', 'quiet', '-ss', str(start),
                        '-t', str(clip_length), '-i', url, '-ar', str(sample_rate),
                        '-ac', '1', '-y', destination_filename],
                       check=True)
